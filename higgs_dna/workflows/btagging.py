...
class BTaggingEfficienciesProcessor(HggSkeletonProcessor):
    def __init__(
        self,
    ...
    def process(self, events: ak.Array) -> Dict[Any, Any]:
        ...
        for variation in variations:
            ...
            btagMVA_selection = {
                ...
                "UParTAK4": {"btagUParTAK4B": jets.btagUParTAK4B} if self.nano_version in [14, 15] else {},
            }
...
class BTaggingEfficienciesHHbbggProcessor(HggSkeletonProcessor):
    def __init__(
        self,
        metaconditions: Dict[str, Any],
        systematics: Dict[str, List[Any]] = None,
    ...
    def process(self, events: ak.Array) -> Dict[Any, Any]:
        ...
        for variation in variations:
            ...
            if self.data_kind == "mc":
                ...
                # Following the recommendations of https://twiki.cern.ch/twiki/bin/view/CMSPublic/SWGuideBTagMCTools for hadronFlavour
                # and the Run 2 recommendations for the bjets
                genJetCondition = (genJets.pt > gen_jet_min_pt) & (numpy.abs(genJets.eta) < 2.5)
                genBJetCondition = genJetCondition & (genJets.hadronFlavour == 5)
                genJets = ak.with_field(genJets, genBJetCondition, "GenIsBJet")
                genCJetCondition = genJetCondition & (genJets.hadronFlavour == 4)
                genJets = ak.with_field(genJets, genCJetCondition, "GenIsCJet")
            ...
            btagMVA_selection = {
                ...
                "particleNet": {"btagPNetB": jets.btagPNetB} if self.nano_version >= 12 else {},
                "robustParticleTransformer": {"btagRobustParTAK4B": jets.btagRobustParTAK4B} if self.nano_version in [12, 13] else {},
                "btagUParTAK4B": {"btagUParTAK4B": jets.btagUParTAK4B} if self.nano_version in [14, 15] else {},
            }
            ...
            # No electron/muon selection applied, as HHbbgg doesn't clean jets with them.
            # Just define them to avoid missing-variables errors.
            electrons = ak.zip(
                {
                    "pt": events.Electron.pt,
                    "eta": events.Electron.eta,
            ...
            # apply the abs(eta) < 2.5 cut to remain consistent with the dijet selection
            jets = jets[numpy.abs(jets.eta) < 2.5]
            jets = jets[ak.argsort(jets.pt, ascending=False)]
            ...
            # Based on recommendations for the tight QCD WP seen here: https://btv-wiki.docs.cern.ch/PerformanceCalibration/#working-points
            if hasattr(self, "bjet_wps") and self.bjet_wps is not None:
                logger.info(f"Deriving b-tagging efficiencies for {dataset_name} ({self.year[dataset_name][0]}) with mva {self.bjet_mva} and WPs {self.bjet_wps}")
                ...
                for wp, btag_WP in btag_WPs.items():
                    bJetCondition = ((jets.pt > gen_jet_min_pt) & (abs(jets.eta) < 2.5) & (jets[btag_mva_column] >= btag_WP))
                    jets_wp = ak.with_field(jets, bJetCondition, "IsBJet")
                    num_bjets = ak.sum(jets_wp["IsBJet"], axis=-1)
                    diphotons[f"NBJet_{wp}"] = num_bjets

                    # Efficiency variables
                    selected_bjets = jets_wp[jets_wp.hFlav == 5]

                    bJetCondition_bjet = (selected_bjets.pt > gen_jet_min_pt) & (numpy.abs(selected_bjets.eta) < 2.5) & (selected_bjets[btag_mva_column] >= btag_WP)
                    jetCondition_bjet = (selected_bjets.pt > gen_jet_min_pt) & (numpy.abs(selected_bjets.eta) < 2.5)
                    num_bjets_bjet = ak.num(selected_bjets[bJetCondition_bjet])
                    diphotons[f"NBJet_GenBJet_{wp}"] = num_bjets_bjet
                    num_jets_bjet = ak.num(selected_bjets[jetCondition_bjet])
                    diphotons[f"NJet_GenBJet_{wp}"] = num_jets_bjet

                    genBJets_selected_bjets_pt_list = ak.flatten(selected_bjets[bJetCondition_bjet].pt.to_list())
                    genJets_selected_bjets_pt_list = ak.flatten(selected_bjets[jetCondition_bjet].pt.to_list())

                    selected_cjets = jets_wp[jets_wp.hFlav == 4]
                    bJetCondition_cjet = (selected_cjets.pt > gen_jet_min_pt) & (numpy.abs(selected_cjets.eta) < 2.5) & (selected_cjets[btag_mva_column] >= btag_WP)
                    jetCondition_cjet = (selected_cjets.pt > gen_jet_min_pt) & (numpy.abs(selected_cjets.eta) < 2.5)
                    num_bjets_cjet = ak.num(selected_cjets[bJetCondition_cjet])
                    diphotons[f"NBJet_GenCJet_{wp}"] = num_bjets_cjet
                    num_jets_cjet = ak.num(selected_cjets[jetCondition_cjet])
                    diphotons[f"NJet_GenCJet_{wp}"] = num_jets_cjet

                    genCJets_selected_cjets_pt_list = ak.flatten(selected_cjets[bJetCondition_cjet].pt.to_list())
                    genJets_selected_cjets_pt_list = ak.flatten(selected_cjets[jetCondition_cjet].pt.to_list())

                    selected_ljets = jets_wp[jets_wp.hFlav == 0]
                    bJetCondition_ljet = (selected_ljets.pt > gen_jet_min_pt) & (numpy.abs(selected_ljets.eta) < 2.5) & (selected_ljets[btag_mva_column] >= btag_WP)
                    jetCondition_ljet = (selected_ljets.pt > gen_jet_min_pt) & (numpy.abs(selected_ljets.eta) < 2.5)
                    num_bjets_ljet = ak.num(selected_ljets[bJetCondition_ljet])
                    diphotons[f"NBJet_GenLJet_{wp}"] = num_bjets_ljet
                    num_jets_ljet = ak.num(selected_ljets[jetCondition_ljet])
                    diphotons[f"NJet_GenLJet_{wp}"] = num_jets_ljet

                    genLJets_selected_ljets_pt_list = ak.flatten(selected_ljets[bJetCondition_ljet].pt.to_list())
                    genJets_selected_ljets_pt_list = ak.flatten(selected_ljets[jetCondition_ljet].pt.to_list())

                    selected_jets_pt_dict.update({
                        f"genBJet_recoBJet_{wp}": genBJets_selected_bjets_pt_list,
                        f"genBJet_recoJet_{wp}": genJets_selected_bjets_pt_list,
                        f"genCJet_recoBJet_{wp}": genCJets_selected_cjets_pt_list,
                        f"genCJet_recoJet_{wp}": genJets_selected_cjets_pt_list,
                        f"genLJet_recoBJet_{wp}": genLJets_selected_ljets_pt_list,
                        f"genLJet_recoJet_{wp}": genJets_selected_ljets_pt_list,
                    })

                    logger.debug("selected_jets_pt_dict: %s", selected_jets_pt_dict)
