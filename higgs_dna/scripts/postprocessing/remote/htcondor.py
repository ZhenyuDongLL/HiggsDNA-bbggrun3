                        ...
                        with open(job_file_executable, "w") as executable_file:
                            ...
                            if _opt.syst:
                                ...
                                for var in var_dict:
                                    ...
                                    os.chdir(SCRIPT_DIR)
                                    logger.info(f"merge_parquet.py --source {IN_PATH}/{file}/{var_dict[var]} --target {OUT_PATH}/merged/{file}/{var_dict[var]}/ --cats {cat_dict_loc} {verbose_str} {skip_normalisation_str} {do_b_weight_normalisation_str} --abs {genBinning_str} {custom_accumulator_str}")
                                    executable_file.write(f"if [ $1 -eq {i} ]; then\n")
                                    executable_file.write(f"    merge_parquet.py --source {IN_PATH}/{file}/{var_dict[var]} --target {OUT_PATH}/merged/{file}/{var_dict[var]}/ --cats {cat_dict_loc} {verbose_str} {skip_normalisation_str} {do_b_weight_normalisation_str} --abs {genBinning_str} {custom_accumulator_str} || exit 107\n")
                                    executable_file.write("exit 0\n")
                                    executable_file.write("fi\n")
                                    i += 1
                                ...
                                i = 1
                                os.chdir(SCRIPT_DIR)
                                print(f"merge_parquet.py --source {IN_PATH}/{file}/nominal --target {OUT_PATH}/merged/{file}/ --cats {cat_dict_loc} {verbose_str} {skip_normalisation_str} {do_b_weight_normalisation_str} --abs {genBinning_str} {custom_accumulator_str}")
                                executable_file.write(f"if [ $1 -eq 0 ]; then\n")
                                executable_file.write(f"    merge_parquet.py --source {IN_PATH}/{file}/nominal --target {OUT_PATH}/merged/{file}/ --cats {cat_dict_loc} {verbose_str} {skip_normalisation_str} {do_b_weight_normalisation_str} --abs {genBinning_str} {custom_accumulator_str} || exit 107\n")
                                executable_file.write("exit 0\n")
                                executable_file.write("fi\n")
