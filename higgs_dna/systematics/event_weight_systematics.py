...
def cTagSF_WPs(events, weights, is_correction=True, year="2017", n_toys=1000, **kwargs):
    """
    Add c-tagging reshaping SFs as from /https://github.com/higgs-charm/flashgg/blob/dev/cH_UL_Run2_withBDT/Systematics/scripts/applyCTagCorrections.py
    BTV scale factor Wiki: https://btv-wiki.docs.cern.ch/ScaleFactors/
    events must contain jet objects, moreover evaluation of SFs works by calculating the scale factors for all the jets in the event,
    ...
    if is_correction:
        ...
        for i in range(max_n_jet):
            ...
            nth_jet_pn_b = choose_jet(events["sel_jets"].pn_b, i, -1)
            ...
            _wp_54 = ak.ones_like(nth_jet_pn_b) * 54
            _wp_53 = ak.ones_like(nth_jet_pn_b) * 53
            _wp_52 = ak.ones_like(nth_jet_pn_b) * 52
            _wp_51 = ak.ones_like(nth_jet_pn_b) * 51
            _wp_50 = ak.ones_like(nth_jet_pn_b) * 50

            _wp_44 = ak.ones_like(nth_jet_pn_b) * 44
            _wp_43 = ak.ones_like(nth_jet_pn_b) * 43
            _wp_42 = ak.ones_like(nth_jet_pn_b) * 42
            _wp_41 = ak.ones_like(nth_jet_pn_b) * 41
            _wp_40 = ak.ones_like(nth_jet_pn_b) * 40

            _wp_0 = ak.zeros_like(nth_jet_pn_b)
            wp = ak.zeros_like(nth_jet_pn_b)
            ...
            if year in (2017, 2018):
                ...
                wp = ak.where(
                    (0.2 < nth_jet_pn_b_plus_c) & (nth_jet_pn_b_plus_c <= 0.5),
                    _wp_41,
                    wp
                )
                wp = ak.where(
                    (0.1 < nth_jet_pn_b_plus_c) & (nth_jet_pn_b_plus_c <= 0.2),
                    _wp_40,
                    wp
                )
                ...
                wp = ak.where(
                    (0.17 < nth_jet_pn_b_plus_c) & (nth_jet_pn_b_plus_c <= 0.35),
                    _wp_41,
                    wp
                )
                wp = ak.where(
                    (0.1 < nth_jet_pn_b_plus_c) & (nth_jet_pn_b_plus_c <= 0.17),
                    _wp_40,
                    wp
                )
        ...
        sfs_up = [ak.values_astype(dummy_sf, float) for _ in ctag_systematics]
        sfs_down = [ak.values_astype(dummy_sf, float) for _ in ctag_systematics]
        ...
        for i in range(max_n_jet):
            ...
            if year in (2017, 2018):
                ...
                wp = ak.where(
                    (0.2 < nth_jet_pn_b_plus_c <= 0.5),
                    _wp_41,
                    wp
                )
                wp = ak.where(
                    (0.1 < nth_jet_pn_b_plus_c <= 0.2),
                    _wp_40,
                    wp
                )
