...
def pass_approximate_frixione_isolation(
    ...
    del gen_particles, gen_mask

    # calulate deltaR between photon and gvalid enparts since we need to remove all gen which has dR bigger than frix cone
    deltaR_genpart_pho = valid_gen_particles.metric_table(gen_photon)

    # since there are events with no photons, and corresponding deltaR_genpart_pho will be None, to remove None we use ak.sigletons
    deltaR_genpart_pho = deltaR_genpart_pho[ak.num(ak.flatten(ak.singletons(deltaR_genpart_pho),axis=-1),axis=-1) > 0]
    ...
    del deltaR_mask, deltaR_genpart_pho
    ...
    del sorted_indices_deltaR_pho_genPart, Et_valid_gen_particles
    ...
    del sorted_Et_valid_gen_particles, sorted_deltaR_pho_genPart

    # Photon ET prodcast is naccessry because some time there is no cumsum_GenEt but photn ET is there and we need to devide cumsum_GenEt by photon ET
    ...
    del pho_Et_broadcasted, cumsum_GenEt

    pass_frix = ak.all(cumsum_GenEt_over_phoET < cone_vars , axis=-1)
    del cone_vars
    return pass_frix, cumsum_GenEt_over_phoET
...
def attach_geninfo_to_photons(
    ...
    isHardProcess = (Genmatched_photon_parent.statusFlags & (1 << 7)) != 0  # isHardProcess
...
def attach_frixione_isolation_flag_to_diphotons(
    ...
    for frix_cone in frix_cones:
        ...
        cumsum_GenEt_Name = f"cumsum_GenEt_over_phoET_R{cone}"

        flag_pass_frix_lead, cumsum_GenEt_lead = pass_approximate_frixione_isolation(self, gen_parts, diphotons["pho_lead"].matched_gen, frix_cone=frix_cone)
        flag_pass_frix_sublead, cumsum_GenEt_sublead = pass_approximate_frixione_isolation(self, gen_parts, diphotons["pho_sublead"].matched_gen, frix_cone=frix_cone)
        ...
        diphotons["pho_lead"] = ak.with_field(diphotons["pho_lead"], cumsum_GenEt_lead, cumsum_GenEt_Name)
        diphotons["pho_sublead"] = ak.with_field(diphotons["pho_sublead"], cumsum_GenEt_sublead, cumsum_GenEt_Name)
        lead_isPythia = (
